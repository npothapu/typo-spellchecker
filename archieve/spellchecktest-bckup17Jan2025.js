...
const { allure } = require('./allure-playwright'); // Import Allure reporting
    ...
    test('Check spelling on all links of the webpage', async ({ page, baseURL }) => {
      ...
      const links = await page.$$eval('a', (anchors) =>
        anchors
          ...
          .filter((href) => href.startsWith(baseURL)) // Only include valid URLs
      );

      console.log('Extracted Links:', links);

      for (const link of links) {
        allure.step(`Checking link: ${link}`, async () => {
          ...
          try {
            ...
            if (misspelledWords.length > 0) {
              ...
              allure.attachment(`Misspelled Words: ${link}`, JSON.stringify(misspelledWords, null, 2), 'application/json');
            } else {
            ...
            allure.step(`Error checking link ${link}`, () => {
